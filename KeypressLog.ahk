; KeypressOSD.ahk
;----------------------------------------------------------
; ChangeLog : v2.10 (2017-01-22) - Added ShowStickyModKeyCount option
...
CreateHotkey() {
	...
	for i, mod in ["Ctrl", "Shift", "Alt", "LWin", "RWin"]
  ...
  if !instr(fileexist("logs"), "D")
   {
	...
	if HotkeyStr contains click,button
     ...
     fileappend, %time% - %HotkeyStr% - %mx% x %my%`n, logs\%name%, UTF-8
	 }
	else
	 {
	   time=%a_dd%/%a_mm%/%a_yyyy% %a_hour%:%a_min%:%a_sec%
     fileappend, %time% - %HotkeyStr%`n, logs\%name%, UTF-8
	 }
}
